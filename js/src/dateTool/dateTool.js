...
KISSY.add('dateTool', function(S, undef) {
	...
	S.augment(DateSetting, {
		...
		refreshFixed: function(fixed){
			...
			fixed.replace(DateSetting.reg_fixed, function(a, b, c){
				if(b && !o[b]){
					o[b] = c;
				}
			});
	...
	function dropSelect(dt){
		var type = $target_ipt[0].className.substring(4);
		setting.item(type, dt.html()).refreshIpts(type);
	...
	function dateSelect(dt){
		setting.item('date', dt.html()).refreshIpts('date').fillTarget();
	...
	//初始化DOM结构, 各个节点的引用
	$div_wrap = $('<div class="hdt-wrap"><div class="hdt-ctrl"><div class="hdt-tips"></div><div class="hdt-btns"><a href="#" class="hdt-clear">清除</a><a href="#" class="hdt-now">现在</a><a href="#" class="hdt-complete">完成</a></div></div><div class="hdt-ipt-list"><input type="text" class="hdt-year" />-<input type="text" class="hdt-month" />-<input type="text" class="hdt-date" /><input type="text" class="hdt-hour" />:<input type="text" class="hdt-minute" />:<input type="text" class="hdt-second" /></div><div class="hdt-week-list"></div><div class="hdt-date-list"></div><div class="hdt-drop-list-wrap"><div class="hdt-drop-list"></div><p class="hdt-drop-list-ctrl"><a href="#">上页</a><a href="#">下页</a><a href="#">关闭</a></p></div></div>');
