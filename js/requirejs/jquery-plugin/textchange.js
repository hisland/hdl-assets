...
var ns = ".inputEvent ",
	...
	bindTo = ["input", "textInput", "propertychange", "paste", "cut", "keydown", "drop", ""].join(ns),
...
$.event.special.textchange = {
	setup: function(data, namespaces, handler) {
		var triggerTimer, bndCount, changeTimer,
		...
		if (elem.tagName in supported) {
			...
			$elem.bind(dlgtTo, function(e) {
				...
				if (target.tagName in supported && !$.data(elem, dataDlg)) {
					...
					if (!bndCount) target.bind(bindTo, handler);
...
$.fn.input = function(handler) {
	return handler ? this.bind("textchange", handler) : this.trigger("textchange");
}
})(jQuery);