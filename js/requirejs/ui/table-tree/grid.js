	...
	function Grid(setting){
		...
		setting.colModel[0] = {
			...
			process: function(row, col){
				var p = $('<p></p>');
				for(var i=0; i<row.pids; i++){
					p.append('<span style="display:inline-block;width:10px;"></span>');
				}
				p.append('<span class="table-grid+">+' + row.mobileName + '</span>');
				return p.children();
			}
		};
		...
		setData: function(data){
			...
			//没有数据
				...
				function sortRows(rows){
					...
					function makeRs(data, pids){
						S.each(data, function(v, i, o){
							v.pids = pids;
							rs.push(v);
							if(v.children){
								makeRs(v.children, pids + 1);
							}
						});
					}
					makeRs(tree_data, 0);
				...
				data.rows = sortRows(data.rows);
				//保存数据引用
				grid.data = data;
				S.each(data.rows, function(row, i, o){
					grid.$tbody.append(makeTr(row).attr('data-id', row.id).attr('data-pid', ' '+row.pid));
				});
			}
