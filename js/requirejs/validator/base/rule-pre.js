define(['./rule', 'kissy'], function(Rule, S){
	...
	Rule.add(
		'ipv4',
		function(str){
			var reg = /^(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)$/;
			if(reg.test(str) && str !== '0.0.0.0'){
				return true;
			}else{
				return false;
			}
		},
		getText('IPv4,点分十进制共四位,每个数字不能超过255,不能为0.0.0.0')
	);
	Rule.add(
		'ipv4-prefix0',
		function(str){
			var reg = /^(25[0-5]|2[0-4]\d|[01]\d{2}|\d?\d)\.(25[0-5]|2[0-4]\d|[01]\d{2}|\d?\d)\.(25[0-5]|2[0-4]\d|[01]\d{2}|\d?\d)\.(25[0-5]|2[0-4]\d|[01]\d{2}|\d?\d)$/;
			var is0 = /^0{1,2}\.0{1,2}\.0{1,2}\.0{1,2}$/;
			if(reg.test(str) && !is0.test(str)){
				return true;
			}else{
				return false;
			}
		},
		getText('IPv4,点分十进制,共四位,每个数字不能超过255,可有前置0,不能为0.0.0.0')
	);
